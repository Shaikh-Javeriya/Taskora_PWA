        ...
        try {
          ...
          if (hasProjectId) {
            ...
            for (const row of cleanData) {
              // get values robustly (case-insensitive)
              ...
              if (!projectId && projectId !== 0) {
                // skip rows that don't have a valid project id (or you can choose to create orphan tasks)
              ...
              const estimated_hours = rawActual ? parseFloat(String(rawActual).replace(/[^0-9.\-]/g, "")) || 0 : 0;
              const billable_hours = rawBillable ? parseFloat(String(rawBillable).replace(/[^0-9.\-]/g, "")) || 0 : 0;

              // Parse date safely (if invalid -> null)
              let deadline = null;
              if (rawDeadline) {
              ...
              // Create task (uses your dbOperations.createTask)
              await dbOperations.createTask({
                title,
                status,
                priority: rawPriority ? String(rawPriority).trim() : "medium",
                estimated_hours,
                billable_hours,
                deadline,
                project_id: projectId,
                description: getRowValue(row, results, "Description") ?? "",
              });
            }
            ...
            // All writes finished — trigger UI reload
            if (typeof window !== "undefined") {
              window.dispatchEvent(new Event("dataUpdated"));
            }
            toast?.success?.("✅ Tasks imported successfully!");
          } else {
            // Import as projects (normalize similarly)
            const projects = cleanData.map(row => {
              ...
              return {
                ...
                actual_hours: rawActual ? parseFloat(String(rawActual).replace(/[^0-9.\-]/g, "")) || 0 : 0,
                billable_hours: rawBillable ? parseFloat(String(rawBillable).replace(/[^0-9.\-]/g, "")) || 0 : 0,
                deadline,
                created_at: new Date(),
                updated_at: new Date()
              };
            ...
            if (typeof window !== "undefined") window.dispatchEvent(new Event("dataUpdated"));
          }
