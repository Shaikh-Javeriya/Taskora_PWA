...
const normalizeProjectStatus = (raw) => {
  ...
  if (s === "todo" || s === "to do" || s === "to-do" || s === "td" || s === "on hold") return "on-hold";
  // anything else (in-progress, active, etc.) counts as active
  return "active";
};
              ...
              if (estimated_hours > 0) {
                if (billable_hours > 0 && billable_hours < estimated_hours) {
                  ...
                  await dbOperations.createTimeEntry({
                    task_id: taskId,
                    project_id: projectId,
                    duration: billable_hours, // in hours
                    billable: true,
                    start_time: new Date(),
                    end_time: new Date(),
                  ...
                  // Non-billable remainder
                  const nonBillable = estimated_hours - billable_hours;
                  if (nonBillable > 0) {
                    await dbOperations.createTimeEntry({
                      task_id: taskId,
                      project_id: projectId,
                      duration: nonBillable, // in hours
                      billable: false,
                      start_time: new Date(),
                      end_time: new Date(),
                  ...
                  // Single entry (either fully billable or fully non-billable)
                  await dbOperations.createTimeEntry({
                    task_id: taskId,
                    project_id: projectId,
                    duration: billable_hours > 0 ? billable_hours : estimated_hours, // in hours
