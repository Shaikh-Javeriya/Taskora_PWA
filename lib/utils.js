    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        try {
          ...
          if (headers.includes("Deadline") && headers.includes("Actual Hours")) {
            // --- PROJECT CSV ---
            const projects = results.data.map(row => ({
              name: row.Title?.trim(),
              status: row.Status?.trim() || "active",
              actual_hours: parseFloat(row["Actual Hours"] || 0),
              billable: row.Billable && row.Billable.toLowerCase() !== "no",
            ...
            const tasks = results.data.map(row => ({
              ...
              status: row.Status?.trim() || "todo",
              actual_hours: parseFloat(row["Actual Hours"] || 0),
              billable: row.Billable && row.Billable.toLowerCase() !== "no",
              deadline: row.Deadline ? new Date(row.Deadline) : null,
              project_id: null // TODO: map to project if you include Project column
            }));
